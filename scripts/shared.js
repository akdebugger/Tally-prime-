...
function renderSidebar() {
    ...
    // REMOVED: initMobileNavigation() from here to prevent premature binding
}
...
// =====================================
// MODULE PROGRESS LIST WITH STATUS BAR
// =====================================
...
function renderModuleProgress() {
    ...
    // 1. Generate the HTML for the thin segmented progress bar pieces
    let segmentsHtml = "";
    for (let i = 0; i < totalTopics; i++) {
        ...
        segmentsHtml += `<div class="progress-segment ${stateClass}"></div>`;
    }

    // 2. Position the tooltip precisely over the center of the active segment
    const tooltipPosition = ((currentIndex + 0.5) / totalTopics) * 100;

    let html = `
        <div class="module-progress-card">
            <h3>${current.module.module}</h3>
            <p>Topic ${currentIndex + 1} of ${totalTopics}</p>
            
            <div class="thin-progress-wrapper">
                <div class="progress-tooltip" style="left: ${tooltipPosition}%">
                    ${currentIndex + 1}/${totalTopics}
                </div>
                <div class="thin-segmented-bar">
                    ${segmentsHtml}
                </div>
            </div>
...
function initMobileNavigation() {
    ...
    // Use Event Delegation: Watch clicks globally across the document
    document.addEventListener("click", e => {
        // Safely check if the clicked element (or its SVG/span child) belongs to #navToggle
        ...
        if (navToggle) {
            ...
            // Clicked outside the navigation element, close it out
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // SAFE: Runs tracking immediately; works seamlessly whenever header.html async finishes loading
